...
const sanitizeProperties = properties => {
  ...
  return {
    ...
    subscribeToCaptions: sanitizeBooleanProperty(properties.subscribeToCaptions ? properties.subscribeToCaptions : false),
...
const sanitizeStreamProperties = streamProperties => {
  each(streamProperties, (individualStreamProperties, streamId) => {
    ...
    if (subscribeToAudio !== undefined) {
      individualStreamProperties.subscribeToAudio = sanitizeBooleanProperty(subscribeToAudio);
    }
    if (subscribeToVideo !== undefined) {
      individualStreamProperties.subscribeToVideo = sanitizeBooleanProperty(subscribeToVideo);
    }
    if (subscribeToCaptions !== undefined) {
      individualStreamProperties.subscribeToCaptions = sanitizeBooleanProperty(subscribeToCaptions);
    }
