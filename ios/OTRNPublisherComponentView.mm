...
- (void)updateProps:(const Props::Shared &)props
  ...
  if (oldViewProps.videoContentHint != newViewProps.videoContentHint) {
    [_impl setVideoContentHint:RCTNSStringFromString(
                                   newViewProps.videoContentHint)];
  }
  ...
  if (oldViewProps.videoBitratePreset != newViewProps.videoBitratePreset) {
    [_impl setVideoBitratePreset:RCTNSStringFromString(
                                     newViewProps.videoBitratePreset)];
  }
...
- (std::shared_ptr<const OTRNPublisherEventEmitter>)getEventEmitter {
  ...
  return std::static_pointer_cast<const OTRNPublisherEventEmitter>(
      _eventEmitter);
}
...
- (void)handleAudioLevel:(float)audioLevel {
  auto eventEmitter = [self getEventEmitter];
  if (eventEmitter) {
    OTRNPublisherEventEmitter::OnAudioLevel payload{.audioLevel = audioLevel};
