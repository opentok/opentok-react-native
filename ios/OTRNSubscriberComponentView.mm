...
#import <react/renderer/components/OpentokReactNative/ComponentDescriptors.h>
#import <react/renderer/components/OpentokReactNative/EventEmitters.h>
#import <react/renderer/components/OpentokReactNative/Props.h>
#import <react/renderer/components/OpentokReactNative/RCTComponentViewHelpers.h>
#endif
...
template <typename T> T makeConnectionStruct(NSDictionary *connectionDict) {
  return T{.creationTime =
               std::string([connectionDict[@"creationTime"] ?: @"" UTF8String]),
           .data = std::string([connectionDict[@"data"] ?: @"" UTF8String]),
           .connectionId = std::string(
               [connectionDict[@"connectionId"] ?: @"" UTF8String])};
}
...
StreamStruct makeStreamStruct(NSDictionary *streamDict) {
  ...
  return StreamStruct{
      ...
      .creationTime =
          std::string([streamDict[@"creationTime"] ?: @"" UTF8String])};
}

using namespace facebook::react;

@interface OTRNSubscriberComponentView
    : RCTViewComponentView <RCTOTRNSubscriberViewProtocol>
@end
...
+ (ComponentDescriptorProvider)componentDescriptorProvider {
  return concreteComponentDescriptorProvider<
      OTRNSubscriberComponentDescriptor>();
}
...
- (void)updateProps:(const Props::Shared &)props
  ...
  const auto &oldViewProps =
      *std::static_pointer_cast<const OTRNSubscriberProps>(_props);
  const auto &newViewProps =
      *std::static_pointer_cast<const OTRNSubscriberProps>(props);
...
- (std::shared_ptr<const OTRNSubscriberEventEmitter>)getEventEmitter {
  ...
  return std::static_pointer_cast<const OTRNSubscriberEventEmitter>(
      _eventEmitter);
}
    ...
    OTRNSubscriberEventEmitter::OnSubscriberDisconnected payload{
        .stream = makeStreamStruct<
            ...
            OTRNSubscriberEventEmitter::
                OnSubscriberDisconnectedStreamConnection>(streamDict)};
  ...
  if (eventEmitter) {
    OTRNSubscriberEventEmitter::OnVideoDisableWarningLifted payload{
        .stream = makeStreamStruct<
            ...
            OTRNSubscriberEventEmitter::
                OnVideoDisableWarningLiftedStreamConnection>(streamDict)};
